---
name: consult
description: Use when the user wants a second opinion from another AI coding agent before deciding — "consult Claude", "hoi y voi Claude", "ask Codex", "debate with Claude", "get a second opinion", "what would Claude/Codex say". Consults a peer agent (Claude, Codex, OpenCode, CommandCode, Gemini) via its CLI with named, resumable Claude sessions, streamed progress, a packaged context prompt, and a final synthesis comparing both viewpoints.
version: 0.1.0
author: Dat Nguyen
---
...
1. Confirm the consultation target: decision to make, current state, constraints, options considered, and what evidence would change the answer.
2. Decide whether to consult. Consult when the user asks directly, when architecture or prompt decisions benefit from an adversarial second view, or when the problem has credible tradeoffs. Skip for trivial commands, purely mechanical edits, or anything requiring secrets.
...
Prefer the bundled helper for non-trivial consultations:
...
- Stores `session_id` by `advisor:name` in `~/.config/consult/sessions.json` and auto-resumes when the same `--name` is reused.
...
Switch advisor with `--agent codex|opencode|commandcode|gemini`. List what's available with `consult --list-advisors`. Add or fix an advisor in `~/.config/consult/advisors.json` (no code change needed). Use `--new-session` to ignore a stored session, `--resume ID_OR_SEARCH` to force a resume target, `--fork-session` to branch, and `--model` to raise the model only when the decision justifies it.

## Prompting The Advisor

Read [references/consultation-protocol.md](references/consultation-protocol.md) when preparing a substantive prompt (architecture, prompt, eval, or debugging decisions).
...
- Preserve long-running calls. Do not abandon a started consultation unless the user cancels it or the process fails.
