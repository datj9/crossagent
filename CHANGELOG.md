...
### Added
- Initial release: `consult` CLI and Agent Skill for getting a second opinion from a peer AI coding agent.
- Cross-agent advisor registry with built-ins for **claude** (full), **codex**, **opencode**, **commandcode**, and **gemini** (experimental).
- User-overridable advisors via `~/.config/consult/advisors.json` — add or fix an advisor with no code change.
...
### Fixed
- Redact consultation prompts from command progress logs for every advisor.
